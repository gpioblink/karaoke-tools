...
var UpdateReading HandlerFunc = func(ctx context.Context, service application.MusicService, req Request) {
	...
	// アドレスを元にファイル番号を特定
	fileIdx := -1
	if addr >= 0x000000000049c800 && addr < 0x000000002049c800 { // 5253120-542124032
		fileIdx = 0
	} else if addr >= 0x000000002049c800+0x2000000 && addr < 0x000000004049c800 {
		fileIdx = 1
	} else if addr >= 0x000000004049c800+0x2000000 && addr < 0x000000006049c800 {
		fileIdx = 2
	} else {
		return
	}
